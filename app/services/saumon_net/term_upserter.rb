module SaumonNet
  class TermUpserter
    ...
    def upsert
      ...
      SubstituteProcessor.new(term, @mandate_data, @session_id).process
    ...
    def map_term_attributes
...

      {
        ...
        start_date: DateParser.parse_datetime(@mandate_data["dateDebut"], session_id: @session_id),
        end_date: DateParser.parse_datetime(@mandate_data["dateFin"], session_id: @session_id),
        publish_date: DateParser.parse_datetime(@mandate_data["datePublication"], session_id: @session_id),
        assumption_date: DateParser.parse_datetime(TermAttributeExtractor.extract_assumption_date(@mandate_data), session_id: @session_id),
        role_rank: @mandate_data["preseance"]&.to_i,
        capacity: capacity,
        main: @mandate_data["nominPrincipale"] == "1",
        origin: TermAttributeExtractor.extract_origin(@mandate_data),
        end_reason: TermAttributeExtractor.extract_end_reason(@mandate_data),
        seat: TermAttributeExtractor.extract_seat_info(@mandate_data),
        collaborators: TermAttributeExtractor.extract_collaborators(@mandate_data)
      }
    end
